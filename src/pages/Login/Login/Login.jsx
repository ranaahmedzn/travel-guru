...
const Login = () => {
    ...
    const handleResetPassword = () => {
        if(email){
            resetPassword(email)
            .then(() => toast.warn("Please check your email.", {position: toast.POSITION.TOP_CENTER}))
            .catch(err => toast.error(err.message, {position: toast.POSITION.TOP_CENTER}))
        }
        else{
            toast.warn("Please give your email", {position: toast.POSITION.TOP_CENTER})
        }
    }
            ...
            <div className='w-4/5 mx-auto border-2 border-gray-300 py-6 px-8 rounded-lg'>
                ...
                <form onSubmit={handleLogin}>
                    <div className="relative z-0 w-full mb-6 group">
                        <input onChange={(e) => setEmail(e.target.value)} type="email" name="email" id="floating_email" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                    ...
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <span onClick={handleResetPassword} className='ml-2 text-sm font-medium text-[#F9A51A] hover:underline hover:cursor-pointer'>Forgot password?</span>
                    </div>
